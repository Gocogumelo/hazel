...
module.exports = class Cache {
  ...
  async cacheReleaseList(url, baseUrl) {
    ...
    // O link da API do GitHub pra um asset privado nao termina com o nome do
    // arquivo (so um ID numerico), e o Squirrel PRECISA do nome com a versao
    // no final pra funcionar. Por isso apontamos cada .nupkg pra uma rota do
    // proprio Hazel (/download/nupkg/<nome>), que resolve e redireciona pro
    // link real (autenticado) na hora em que o app realmente for baixar.
    for (let i = 0; i < matches.length; i += 1) {
      const proxiedUrl = `${baseUrl}/download/nupkg/${matches[i]}`
      content = content.replace(matches[i], proxiedUrl)
    }
    return content
  }
  ...
  async refreshCache() {
    ...
    for (const asset of release.assets) {
      ...
      if (name.endsWith('.nupkg')) {
        this.latest.nupkgAssets[name] = { api_url: url }
      }
