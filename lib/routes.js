...
module.exports = ({ cache, config }) => {
  ...
  // Serve o .nupkg (pacote completo do Squirrel.Windows) por nome de
  // arquivo -- necessario porque o link da API do GitHub sozinho nao tem
  // o nome/versao do arquivo, que o Squirrel exige pra funcionar.
  exports.downloadNupkg = async (req, res) => {
    const { filename } = req.params
    const latest = await loadCache()
    const asset = latest.nupkgAssets && latest.nupkgAssets[filename]

    if (!asset) {
      send(res, 404, 'The specified file was not found')
      return
    }
